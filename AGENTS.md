# FireRaccoon for agents

> **Recommended:** drive FireRaccoon through the **MCP server** (`fireraccoon_mcp`).
> For REST discovery, read `openapi.yaml` at the repo root.

FireRaccoon is a Flutter client for [Firefly III](https://www.firefly-iii.org/) with
...
1. **Read the schema.** Run `dart run packages/mcp/bin/fireraccoon_mcp.dart schema`
   (or `fireraccoon_mcp schema` when compiled). It prints JSON describing every
...
3. **Get an agent key.** Issue one in Settings under MCP. Its owner can read it back later from the same panel, and
   ...
   Set `FIRERACCOON_URL` and `FIRERACCOON_API_KEY`. Firefly III credentials are
...
result carries a `fireraccoon` block naming the account the key resolved to and
whether it has write access.

## Deployment modes

- `FIRERACCOON_MODE=local` (default): device secure storage; browser/desktop PAT
- `FIRERACCOON_MODE=server` (Docker): encrypted `DATA_DIR`; set
  `DATA_PASSWORD` to unlock on boot; users only enter account passwords;
  volume `fireraccoon_data`
...
| `verify_backup` | Check a backup opens and matches its manifest, then report how the ledger has drifted from it; writes nothing |
...
| `update_recurrence` | Change a recurring rule; fields left out keep their stored value, and the answer names the rows written ahead from it |
| `delete_recurrence` | Delete a recurring rule; transactions it created are kept unless asked otherwise |
...
# TCP
dart run packages/mcp/bin/fireraccoon_mcp.dart --tcp --port 8787

# Schema export
dart run packages/mcp/bin/fireraccoon_mcp.dart schema
```
...
same Settings section and run `fireraccoon_mcp` separately.
